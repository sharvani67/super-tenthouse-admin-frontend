...
const AdminOrders: React.FC = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      if (filterType === 'all' || filterType === 'salesman') {
        const res = await axios.get(`${BASE_URL}/api/salesman-orders/`, { headers });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                        ...
                        {order.event_type && (
                          <div>
                            <div className="flex items-center gap-1">
                              <span className="text-lg">{getEventIcon(order.event_type)}</span>
                              <span className="text-sm font-medium text-gray-800">{order.event_type}</span>
                            </div>
                            {order.venue && (
                              <p className="text-xs text-gray-500">📍 {order.venue}</p>
                            )}
                            {order.guest_count && (
                              <p className="text-xs text-gray-400">👥 {order.guest_count} guests</p>
                            )}
                          </div>
                        )}
                      </td>
      ...
      {/* Order Details Modal - Enhanced with Side by Side Layout */}
            ...
            <div className="p-6 border-t bg-gray-50">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="text-right space-y-1">
                  ...
                  {selectedOrder.tax && formatPrice(selectedOrder.tax) > 0 && (
                    <div className="flex justify-between gap-8 text-sm text-gray-600">
                      <span>Tax:</span>
                      <span>₹{formatPrice(selectedOrder.tax).toFixed(2)}</span>
                    </div>
                  )}
